...
use super::editor_paths::document_instance_index_by_path_unique;
use super::instance_api::{self, InstanceSelector, PropertyPredicate, PropertyScope};
use super::settings_bytecode::SettingsBytecode;

pub(super) fn bytecode_selector_specified(
    index: Option<usize>,
...
pub(super) fn bytecode_selector<'a>(
    index: Option<usize>,
...
pub(super) struct BytecodeInstanceTarget<'a> {
    ...
    pub(super) settings_id: Option<&'a str>,
    pub(super) name: Option<&'a str>,
    pub(super) class_name: Option<&'a str>,
}

pub(super) fn resolve_bytecode_instance_index(
...
pub(super) fn bytecode_parent_index(
    document: &SettingsBytecode,
    no_parent: bool,
    index: Option<usize>,
    settings_id: Option<&str>,
...
pub(super) fn parse_property_scope(raw: &str) -> Result<PropertyScope> {
...
pub(super) fn parse_property_predicates(raw: &[String]) -> Result<Vec<PropertyPredicate>> {
    raw.iter()
        .map(|item| {
...
pub(super) fn parse_property_assignments(raw: &[String]) -> Result<Map<String, Value>> {
    let mut out = Map::new();
    for item in raw {
