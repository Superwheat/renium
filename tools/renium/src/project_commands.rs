...
use super::settings_bytecode::{SettingsBytecode, instance_settings_id};
...
use super::snapshot_types::{ServiceState, SnapshotInstance};
use super::sourcemap::{path_to_sourcemap_relative, write_project_sourcemap_with_updates};
fn structural_source_root(
    ...
    if let Some(project) = project {
        let loaded = project_config::load_project(Some(project), None)?;
        return Ok(loaded
            .root
...
fn structural_project(
    ...
    project_root: &Path,
    src_root: Option<&Path>,
) -> Result<Option<project_config::LoadedProject>> {
    if src_root.is_some() {
        return Ok(None);
    }
...
pub(super) fn load_structural_project(
    project: Option<&Path>,
    project_root: &Path,
) -> Result<project_config::LoadedProject> {
    if let Some(project) = project {
        return project_config::load_project(Some(project), None);
    }
    if let Some(loaded) = project_config::try_load_project(None, Some(project_root))? {
        return Ok(loaded);
    }
    let (root, source_root) = configured_project_layout(project_root, Path::new("src"))?;
    let project = project_config::ReniumProject {
        source_root,
        ..project_config::ReniumProject::default()
    };
    Ok(project_config::LoadedProject {
        path: root.join(project_config::PROJECT_FILE_NAME),
        root,
        project,
    })
}
...
fn projected_structural_target(
    stage: &project_config::ProjectionStage,
    service: &str,
...
fn projected_structural_store(
    ...
    if reject_declarative_target && project_config::project_target_is_declarative(loaded, &target)?
    {
        bail!(
    ...
    let store = project_config::project_structural_store(loaded, &target)?;
...
pub(super) fn clone_instance_command(
...
pub(super) fn rename_instance_command(
...
pub(super) fn desync_package_link_command(
...
pub(super) fn import_model_command(
...
pub(super) fn export_model_command(
...
pub(super) fn import_path_command(
    ...
    let (destination, src_dir) = if let Some(path_json) = args.path_json.as_deref() {
        ...
        project_config::validate_relative_portable_path(destination, "destination")?;
    ...
    if args.push {
        ...
        let projection = project_config::stage_project(project)?;
...
fn validate_import_destination_ownership(
    loaded: &project_config::LoadedProject,
    projection: &project_config::ProjectionStage,
    destination: &Path,
) -> Result<()> {
    let relatives = project_config::project_source_to_staged_relatives(loaded, destination)?;
    ...
    let naming = project_config::project_script_naming(&loaded.project);
    for relative in relatives {
        ...
        if project_config::project_target_is_declarative(loaded, &target)? {
            bail!(
        ...
        let resolved = project_config::resolve_project_write_path(
            loaded,
            &target.iter().collect::<PathBuf>(),
        )?;
...
fn import_path_json_destination(
    ...
    project_config::validate_relative_portable_path(&relative, "--path-json")?;
    let resolution = project_config::resolve_project_write_path(loaded, &relative)?;
...
fn import_source_suffix(
    source: &Path,
    naming: Option<&project_config::ProjectScriptNaming>,
) -> Result<String> {
    let name = source
        .file_name()
        .and_then(|value| value.to_str())
...
pub(super) fn syncback_command(args: SyncbackArgs, global_project: Option<&Path>) -> Result<()> {
    ...
    for service in &services {
        ...
        for (index, instance) in state.instances.iter().enumerate() {
            ...
            if project_config::filter_allows(
...
fn import_filtered_syncback(
    ...
    project_config::refresh_script_naming(&loaded.root)?;
