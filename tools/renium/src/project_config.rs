...
pub fn run_explain_path(args: ExplainPathArgs, global_project: Option<&Path>) -> Result<()> {
    ...
    for service in fs::read_dir(projection.root())? {
        ...
        let source_paths = crate::editor_paths::build_editor_source_paths_by_index(
            &document,
            &service_name,
            &service.path(),
        );
...
fn normalize_rojo_node_value(
    ...
    let normalized = crate::rbx_encode::normalize_project_typed_value(
...
fn print_json(value: &Value, pretty: bool) -> Result<()> {
    ...
    if crate::output::global_pretty_output(pretty) {
