...
fn serialize_instance<'dom, W: Write>(
    ...
    for (property_name, value) in property_buffer.drain(..) {
        ...
        if let Some(serialized_descriptor) = maybe_serialized_descriptor {
            ...
            //
            // ContentId -> Content migrations are intentionally written using
            // the legacy XML property name. Roblox Studio's XML place/model
            // files still serialize properties such as MeshPart.MeshId,
            // MeshPart.TextureID, and SurfaceAppearance.ColorMap as
            // `<Content name="...">`, even though the reflection database now
            // maps them to newer `*Content` runtime properties. Emitting those
            // newer names in `.rbxlx` / `.rbxmx` breaks asset restoration for
            // older on-disk consumers, so keep the legacy property name while
            // still upgrading the XML value type from ContentId to Content.
            ...
            } = &serialized_descriptor.kind
            {
                ...
                if let Ok(new_value) = migration.perform(&converted_value) {
                    if should_write_migrated_content_with_legacy_xml_name(
                        &converted_value,
                        &new_value,
                    ) {
                        write_value_xml(writer, state, serialized_name, &new_value)?;
                    } else {
...
fn should_write_migrated_content_with_legacy_xml_name(
    old_value: &Variant,
    new_value: &Variant,
) -> bool {
    matches!(old_value, Variant::ContentId(_)) && matches!(new_value, Variant::Content(_))
}
