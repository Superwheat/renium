...
Project commands such as `find`, `bg`, and `bs` use saved files, not the live Studio tree. Use edit-context `rbx l` for a live Studio change, then pull it; push saved file changes to Studio. `src/<Service>/...` maps to that Roblox service, so query the service named by the file path.
