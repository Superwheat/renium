...
use super::bytecode_edit::bytecode_service_name;
use super::bytecode_explorer::{
...
pub(super) fn bytecode_apply_property_batch(args: BytecodeApplyPropertyBatchArgs) -> Result<()> {
    ...
    let filter_direction = match args.direction.trim().to_ascii_lowercase().as_str() {
        "studio-to-files" | "studio" => project_config::FilterDirection::StudioToFiles,
        "files-to-studio" | "files" => project_config::FilterDirection::FilesToStudio,
        value => bail!(
    ...
    let mut result = if project_config::project_requires_temporary_stage(&loaded)? {
            ...
            (stage.loaded.as_ref(), stage.projection.as_ref())
        {
            for (_, source_path) in &mut result.source_paths {
                ...
                let canonical =
                    project_config::staged_path_to_project_source(stage_loaded, relative)?
        ...
        let projection = project_config::stage_project(&loaded)?;
...
fn evaluate_property_batch_filters(
    ...
    filter_direction: project_config::FilterDirection,
    document: &SettingsBytecode,
    service: &str,
    instance_index: usize,
    entries: &[ResolvedBytecodePropertyBatchEntry],
