...
use crate::automation_runtime::automation_command;
...
use crate::snapshot_export::export_snapshots;
use crate::snapshot_import::{import_service, import_snapshots};
use crate::sourcemap::generate_sourcemap_command;
...
use crate::version_control::{vc_init, vc_merge, vc_textconv, view_command};
use crate::workflows;
...
pub(crate) fn dispatch(command: Commands, project: Option<&Path>) -> Result<()> {
    match command {
        ...
        Commands::Adapters(args) => project_config::run_adapters(args, project),
        Commands::ImportRojo(args) => project_config::run_import_rojo(args),
        ...
        Commands::Update(args) => lifecycle::run_update(args),
        Commands::UpdateHelper(args) => lifecycle::run_update_helper(args),
