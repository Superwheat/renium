...
use crate::build_info::{GIT_HASH, TIMESTAMP_UNIX, VERSION};
use crate::daemon_control::local_app_data_daemon_path;
use crate::place_target::place_filter;
...
fn report_directory() -> PathBuf {
    if let Ok(current) = std::env::current_dir()
        && let Ok(project) = project_config::load_project(None, Some(&current))
    {
        return project
            .root
            .join(".renium")
