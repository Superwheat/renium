...
pub(super) fn stabilize_reference_indices_with_paths<'a>(
    ...
    crate::settings::bytecode::stabilize_reference_objects(record, |object, index| {
...
fn restore_project_owned_fields(
    ...
    let mut canonical = if destination.is_dir() {
            ...
            .as_deref()
        {
            ...
            Some("rbxm" | "rbxmx") if destination.is_file() => Some(
                crate::rbx::model::read_settings_model_document(destination)?,
            ),
            _ => None,
        }
    };
...
fn reverse_model_bytes(
    ...
    crate::rbx::model::encode_settings_model(&model, binary)
}

fn restore_reverse_model_topology(destination: &Path, model: &mut SettingsBytecode) -> Result<()> {
    let canonical = crate::rbx::model::read_settings_model_document(destination)?;
...
fn reverse_script_plan(
    ...
    for index in &roots {
        let directory = if roots.len() == 1 {
            ...
            root.join(crate::system::files::unique_child_stem(
...
fn plan_reverse_script_node(
    ...
    for child_position in 0..child_count {
        ...
        let child_stem = crate::system::files::unique_child_stem(
