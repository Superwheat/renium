...
pub(crate) fn parse_requested_fields(raw: Option<&str>) -> Option<HashSet<String>> {
...
pub(crate) fn bytecode_explorer_batch_result(args: BytecodeExplorerBatchArgs) -> Result<Value> {
    ...
    if let (Some(loaded), Some(stage)) = (loaded_project.as_ref(), projection.as_ref()) {
        for settings_file in &mut service_settings_files_by_index {
            ...
            *settings_file = config::project_staged_path_to_source(loaded, relative).ok();
        }
...
pub(crate) struct BytecodeNodeProjection<'a> {
...
pub(crate) fn explorer_search_groups(query: &str) -> Vec<Vec<String>> {
...
pub(crate) fn explorer_search_instance_matches(
    document: &SettingsBytecode,
...
impl ExplorerDaemonState {
    ...
    fn reload_services(&mut self, services: &[String]) -> Result<()> {
        let loaded = config::try_load_project(None, Some(&self.project_root))?;
        ...
        let projection = loaded
            .as_ref()
            .map(|loaded| config::stage_project_cached(loaded, &changed_sources))
            .transpose()?;
        let active_root = projection
            .as_ref()
            .map_or(self.src_root.as_path(), config::ProjectionStage::root);
...
pub(crate) fn explorer_daemon_services(src_root: &Path, raw: &str) -> Result<Vec<String>> {
...
pub(crate) fn watch_parent_and_exit(parent_pid: u32) {
...
pub(crate) fn explorer_daemon(args: ExplorerDaemonArgs) -> Result<()> {
