...
use super::rbx_model::canonicalize_settings_reference_stores;
...
pub(super) const LARGE_SERVICE_DETERMINISTIC_FETCH_MIN_INSTANCES: usize = 20_000;
...
pub(super) fn parse_place_guard_config(text: &str, path: &Path) -> Result<PlaceGuardConfig> {
...
pub(super) fn validate_bridge_info(info: &BridgeInfoPayload) -> Result<()> {
...
pub(super) fn is_supported_bridge_codec(value: &str) -> bool {
...
pub(super) struct ExportProjectStage {
    pub(super) project_root: PathBuf,
    container: PathBuf,
    pub(super) import_project_root: PathBuf,
    pub(super) import_src_dir: PathBuf,
    publish_paths: Vec<PathBuf>,
    publish_baseline: BTreeMap<PathBuf, PublishEntryState>,
    pub(super) loaded: Option<project_config::LoadedProject>,
    pub(super) projection: Option<project_config::ProjectionStage>,
    active: bool,
}

impl ExportProjectStage {
    pub(super) fn create(project_root: &Path, src_dir: &Path, services: &[String]) -> Result<Self> {
        let started = Instant::now();
        ...
        if let Some(loaded) = loaded.as_ref() {
            ...
            for mount in &loaded.project.mounts {
                ...
                if mount.ownership != project_config::MountOwnership::ReadOnly {
                ...
                if project_path_is_nested(&source) && source.is_file() {
                    collect_nested_project_paths(
                        ...
                        &mut publish_paths,
                        &mut nested_projects,
                        mount.ownership != project_config::MountOwnership::ReadOnly,
                    )?;
                }
            }
            for adapter in &loaded.project.adapters {
                clone_paths.push(adapter.source.clone());
                if adapter.direction != project_config::AdapterDirection::ToProject {
        ...
        let staged_loaded = if let Some(original) = loaded.as_ref() {
            ...
            Some(project_config::load_project(
        ...
        let projection = staged_loaded
            .as_ref()
            .map(project_config::stage_project)
            .transpose()?;
    ...
    pub(super) fn finish_projection(&self) -> Result<()> {
    ...
    pub(super) fn preview_operations(&self, project_root: &Path) -> Result<Vec<Value>> {
    ...
    pub(super) fn publish(mut self, project_root: &Path) -> Result<Vec<PathBuf>> {
        let started = Instant::now();
        let current = collect_publish_hashes(project_root, &self.publish_paths)?;
...
fn collect_nested_project_paths(
    ...
    let loaded = project_config::load_project(Some(&project_path), None)?;
    ...
    for mount in &loaded.project.mounts {
        ...
        clone_paths.push(relative(&path)?);
        let mount_writable =
            writable && mount.ownership != project_config::MountOwnership::ReadOnly;
        if mount_writable {
            publish_paths.push(relative(&path)?);
        }
        if project_path_is_nested(&path) && path.is_file() {
    ...
    for adapter in &loaded.project.adapters {
        ...
        if writable && adapter.direction != project_config::AdapterDirection::ToProject {
        ...
        if project_path_is_nested(&source) && source.is_file() {
            collect_nested_project_paths(
                ...
                publish_paths,
                visited,
                writable && adapter.direction != project_config::AdapterDirection::ToProject,
            )?;
        }
        if let Some(output) = adapter.output.as_ref() {
...
pub(super) fn collect_publish_hashes(
    root: &Path,
...
pub(super) fn publish_operation_paths(
...
pub(super) fn export_snapshots(mut args: ExportSnapshotsArgs) -> Result<()> {
...
pub(super) fn export_snapshots_with_warm_bridge(
...
pub(super) fn parse_bridge_ports(raw: &str) -> Result<Vec<u16>> {
...
pub(super) fn parse_bridge_chunk(value: Value) -> Result<BridgeChunk> {
...
pub(super) fn validate_bridge_chunk(chunk: &BridgeChunk) -> Result<()> {
...
pub(super) fn log_chunk_fetch_metrics(label: &str, metrics: ChunkFetchMetrics) {
...
pub(super) fn fetch_text_chunks<F>(
    chunk_size: usize,
    mut fetcher: F,
...
pub(super) fn fetch_json_payload<F>(
    chunk_size: usize,
    mut fetcher: F,
...
pub(super) fn fetch_typed_payload_with_size<T, F>(
    chunk_size: usize,
    fetcher: F,
