...
pub(super) fn service_settings_path(service_dir: &Path) -> PathBuf {
...
pub(super) fn write_utf8_file(path: &Path, content: &str) -> Result<()> {
    write_bytes_if_changed(path, content.as_bytes())
}

pub(super) fn write_bytes_if_changed(path: &Path, content: &[u8]) -> Result<()> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)
            .with_context(|| format!("Failed to create {}", parent.display()))?;
    }
...
pub(super) fn write_bytes_if_changed_in_existing_dir(path: &Path, content: &[u8]) -> Result<()> {
...
pub(super) fn write_json_streaming<T: Serialize>(path: &Path, value: &T) -> Result<()> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)
            .with_context(|| format!("Failed to create {}", parent.display()))?;
    }
...
pub(super) fn unique_child_stem(
    raw_name: &str,
...
pub(super) fn absolutize_for_daemon(path: &Path) -> PathBuf {
    if path.is_absolute() {
...
pub(super) fn ensure_existing_ancestor_inside(
    root: &Path,
    target: &Path,
    label: &str,
) -> Result<()> {
...
pub(super) fn strip_extended_prefix(path: PathBuf) -> PathBuf {
    if cfg!(windows)
...
pub(super) fn canonical_path(path: &Path) -> std::io::Result<PathBuf> {
    Ok(strip_extended_prefix(fs::canonicalize(path)?))
}

pub(super) fn resolve_existing_project_root(path: &Path) -> Result<PathBuf> {
    canonical_path(path)
        .with_context(|| format!("Failed to resolve project root: {}", path.display()))
}

pub(super) fn resolve_project_root_if_present(path: &Path) -> Result<PathBuf> {
    if !path.exists() {
...
pub(super) fn resolve_link_project_root(raw: &Path) -> Result<PathBuf> {
