...
local function makeBurstYielder(checkInterval: number?, budgetSeconds: number?)
...
local function getParallelChunkWorkerCount(totalItems: number, minItems: number): number
...
local function runParallelChunks(totalItems: number, workerCount: number, job: (number, number) -> ())
...
local function getCachedRefPathSegments(state: ServiceState, instance: Instance): { string }?
	if instance == game then
		return {}
	end
	if not instance:IsDescendantOf(game) then
	...
	if parent ~= nil and parent ~= game then
		local parentSegments = getCachedRefPathSegments(state, parent)
...
local function serializeRefValue(state: ServiceState?, instance: Instance): any
	if state ~= nil then
		...
		local pathSegments = getCachedRefPathSegments(state, instance)
...
local function ensureScriptIndex(state: ServiceState)
	...
	local yieldIfNeeded = makeBurstYielder()
...
local function exportInstanceInternal(
	...
	if includePathData ~= false then
		entry.path = path
		entry.pathSegments = getCachedRefPathSegments(state, instance)
...
local function compactInstanceEntry(state: ServiceState, entry: { [string]: any }): { any }
	...
	if type(properties) == "table" and next(properties) ~= nil then
		...
		for propertyName, _ in pairs(properties) do
			if emitted[propertyName] ~= true then
	...
	return {
		...
		entry.instanceId or false,
		entry.parentInstanceId or false,
	}
end
local function prepareService(serviceName: string): { [string]: any }
	...
	local classSeen = {}
	local classNames = {}
	classSeen[service.ClassName] = true
	classNames[1] = service.ClassName
	...
	local scriptKeyYieldIfNeeded = makeBurstYielder()
	for _, inst in ipairs(scriptObjects) do
		getCachedScriptSourceKey(state, inst)
		scriptKeyYieldIfNeeded()
	end
	if PRE_SERIALIZE_ON_PREPARE and instanceCount <= PRE_SERIALIZE_MAX_INSTANCES then
		local serialized = table.create(instanceCount)
		local workerCount = getParallelChunkWorkerCount(instanceCount, PARALLEL_PRE_SERIALIZE_MIN_ITEMS)
		runParallelChunks(instanceCount, workerCount, function(startIndex, endIndex)
	...
	return {
		...
		preSerialized = state.serializedInstances ~= nil,
...
local function getInstanceBatch(serviceName: string, startIndex: number?, maxCount: number?): string
	...
	if startPos > total then
		...
		if state.serializedInstances then
			...
			local workerCount = getParallelChunkWorkerCount(count, PARALLEL_INSTANCE_BATCH_MIN_ITEMS)
			runParallelChunks(count, workerCount, function(startOffset, endOffset)
...
local function getInstanceBatchCompact(serviceName: string, startIndex: number?, maxCount: number?): string
	local state = getState(serviceName)
	local key = "compact:" .. tostring(startIndex or 1) .. ":" .. tostring(maxCount or 300)
	local cachedPayload = state.batchCacheByKey[key]
	if cachedPayload then
		return cachedPayload
	end
	...
	if startPos > total then
		...
		if state.serializedInstances then
			for i = startPos, finish do
				items[#items + 1] = compactInstanceEntry(state, state.serializedInstances[i])
			end
		else
			local workerCount = getParallelChunkWorkerCount(count, PARALLEL_INSTANCE_BATCH_MIN_ITEMS)
			runParallelChunks(count, workerCount, function(startOffset, endOffset)
				for offset = startOffset, endOffset do
					...
					local instanceId = getCachedInstanceId(state, inst)
					local parentInstanceId = getCachedParentInstanceId(state, inst)
					items[offset] = compactInstanceEntry(state, exportInstanceWithoutSharedFallback(
						state,
						inst,
						"",
						nil,
...
local function getSourceBatchEncoded(serviceName: string, instancePaths: { any }): string
	local state = getState(serviceName)
	ensureScriptIndex(state)
	...
	local cacheKey = table.concat(normalizedPaths, "\0")
	...
	local workerCount = getParallelChunkWorkerCount(#normalizedPaths, PARALLEL_SOURCE_BATCH_MIN_ITEMS)
	runParallelChunks(#normalizedPaths, workerCount, function(startIndex, endIndex)
		for i = startIndex, endIndex do
			local instancePath = normalizedPaths[i]
			local src = state.scriptSources[instancePath]
			if src == nil then
				local scriptInstance = state.scriptInstances and state.scriptInstances[instancePath] or nil
				if scriptInstance == nil then
	...
	for i, instancePath in ipairs(normalizedPaths) do
		local src = sourcesByIndex[i] or ""
		state.scriptSources[instancePath] = src
		out[instancePath] = src
	end
...
local function getSourceBatchChunk(
	...
	local encoded = getSourceBatchEncoded(serviceName, instancePaths)
...
local function handleMethod(method: string, params: { [string]: any }?): any
	...
	elseif method == "getInstanceBatchCompactChunk" then
		local encoded = getInstanceBatchCompact(tostring(p.service), p.startIndex, p.maxCount)
		return chunkEncodedString(encoded, p.chunkStart, p.maxLen)
