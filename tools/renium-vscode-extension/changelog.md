...
- A push reports the properties Studio exposes to no plugin API (`unsupportedProperties` in the result and a warning on stderr) instead of counting them silently or failing on the first one as "not retained"; the files keep those values but the open place cannot receive them.
- Service settings Studio reports only through its native capture (such as `Lighting.Technology`, `Workspace.StreamingMinRadius` or `SoundService.VolumetricAudio`) now take part in pulls, Live Sync reconciles and pushes when both sides carry them; a pull no longer keeps a value in the files that the open place never had.
