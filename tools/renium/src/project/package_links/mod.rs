...
use crate::editor::document::editor_child_by_stem;
...
fn read_link_source_meta(source_path: &Path) -> LinkSourceMeta {
    ...
    if source_path.is_file() {
        let root_class = source_path
            ...
            .and_then(|name| infer_source_script(name, &config::ProjectScriptNaming::default()))
...
fn resolve_link_target_storage(
    ...
    let loaded = config::try_load_project(None, Some(project_root))?.filter(|loaded| {
        ...
        loaded.as_ref()
    {
        if enforce_projection_ownership {
            ...
            if config::project_target_is_declarative(loaded, &staged_segments)? {
                bail!(
        ...
        let resolved = config::resolve_project_write_segments(loaded, &staged_segments)?;
...
fn link_script_file_names(
    ...
    let extension = match naming.extension {
        ...
        config::ScriptExtensionPolicy::Preserve => source_file_name
            .rsplit_once('.')
...
#[derive(Default)]
pub(crate) struct LinkEnforcement {
    mirror_to_canonical: HashMap<String, (PathBuf, bool)>,
    canonical_to_mirrors: HashMap<String, Vec<PathBuf>>,
    pub(crate) read_only_packages: Vec<ReadOnlyPackageEnforcement>,
    active: bool,
}

pub(crate) struct ReadOnlyPackageEnforcement {
    pub(crate) link_id: String,
    pub(crate) service: String,
    pub(crate) target_segments: Vec<String>,
    pub(crate) target_ordinals: Vec<usize>,
...
impl LinkEnforcement {
    ...
    pub(crate) fn read_only_package_for_path(
        &self,
        service: &str,
        path_segments: &[String],
        path_ordinals: &[usize],
    ...
    pub(crate) fn reject_read_only_package_path(
        &self,
        service: &str,
        path_segments: &[String],
        path_ordinals: &[usize],
...
pub(crate) fn build_loaded_project_link_enforcement(
...
pub(crate) fn build_link_enforcement(
    project_root: &Path,
    src_root: &Path,
...
pub(crate) fn apply_link_enforcement_to_changed_paths(
    project_root: &Path,
...
fn link_project_naming(project_root: &Path) -> Result<config::ProjectScriptNaming> {
    Ok(config::try_load_project(None, Some(project_root))?
        .filter(|loaded| loaded.root == project_root)
        .map(|loaded| config::project_script_naming(&loaded.project))
        .unwrap_or_default())
}
