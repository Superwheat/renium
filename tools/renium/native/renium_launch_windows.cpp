...
// Window messages also arrive when Explorer/Alt-Tab selects Studio. They do
// not carry Studio's original input-message provenance, and the foreground
// can still belong to the previous app. Only constrain messages generated by
// a background API call made by Studio itself, never all incoming messages.
static thread_local unsigned backgroundWindowChangeDepth = 0;
// Set once Windows activates a Studio window outside any call Studio made
// itself: the user selected Studio from the taskbar, Alt-Tab or a click.
static volatile LONG userSelectedStudio = 0;

// Studio's own background show/raise calls must not cover the user's app,
// so before the user ever selects Studio they sink to the bottom. Once the
// user has selected it, the same calls may arrive while Windows is still
// moving the foreground over, and sinking would undo that selection: from
// then on the order is only frozen.
static void constrainBackgroundOrder(HWND& after, UINT& flags)
{
    flags |= SWP_NOACTIVATE;
    if (flags & SWP_NOZORDER) return;
    if (userSelectedStudio) flags |= SWP_NOZORDER;
    else after = HWND_BOTTOM;
}
struct BackgroundWindowChange
{
    const bool blocked;
    explicit BackgroundWindowChange(bool value) : blocked(value)
    {
        if (blocked) ++backgroundWindowChangeDepth;
    }
    ~BackgroundWindowChange()
    {
        if (blocked) --backgroundWindowChangeDepth;
    }
    BackgroundWindowChange(const BackgroundWindowChange&) = delete;
    BackgroundWindowChange& operator=(const BackgroundWindowChange&) = delete;
};
...
static int backgroundShowCommand(HWND window, int command)
{
    if (command == SW_HIDE) return command;
    ...
    return minimized ? SW_SHOWMINNOACTIVE : SW_SHOWNA;
}

// While a place settles, Studio hides its main window and shows it again.
// Hiding the foreground window hands the foreground to the user's previous
// app, so a re-show that may not activate would leave Studio behind the app
// the user had just left. A window Studio hid while it held the foreground
// may activate again when Studio shows it.
// The same applies when Studio destroys the window the user had in front and
// creates a replacement: the next top-level window Studio shows may activate.
static HWND foregroundHiddenWindow = nullptr;
static bool foregroundReplaced = false;
static ULONGLONG foregroundLostAt = 0;

static bool holdsForeground()
{
    DWORD foreground = 0;
...
static void noteForegroundLoss(HWND window, bool replaced)
{
    if (!holdsForeground() || (GetWindowLongPtrW(window, GWL_STYLE) & WS_CHILD)) return;
    foregroundHiddenWindow = replaced ? nullptr : window;
    foregroundReplaced = replaced;
    foregroundLostAt = GetTickCount64();
}

static bool restoresUserForeground(HWND window, int command)
{
    if (command == SW_HIDE)
    {
        noteForegroundLoss(window, false);
        return false;
    }
    if (!foregroundLostAt || GetTickCount64() - foregroundLostAt > 15000) return false;
    if (GetWindowLongPtrW(window, GWL_STYLE) & WS_CHILD) return false;
    if (!foregroundReplaced && window != foregroundHiddenWindow) return false;
    foregroundHiddenWindow = nullptr;
    foregroundReplaced = false;
    foregroundLostAt = 0;
    return true;
}
...
{
    const bool blocked = !restoresUserForeground(window, command) && command != SW_HIDE
        && backgroundRequest() && !(GetWindowLongPtrW(window, GWL_STYLE) & WS_CHILD);
    const BackgroundWindowChange change(blocked);
    char operation[40];
    ...
    return ShowWindow(window, backgroundShowCommand(window, command));
}

static BOOL WINAPI launchSetWindowPlacement(HWND window, const WINDOWPLACEMENT* placement)
{
    const bool blocked = !(placement && restoresUserForeground(window, int(placement->showCmd)))
        && backgroundRequest();
    const BackgroundWindowChange change(blocked);
    // Qt also restores windows through this API. Preserve its placement data,
    // with the same deferred activation policy as ShowWindow.
    if (!blocked || !placement) return SetWindowPlacement(window, placement);
    WINDOWPLACEMENT background = *placement;
    background.showCmd = backgroundShowCommand(window, int(background.showCmd));
    return SetWindowPlacement(window, &background);
}

static BOOL WINAPI launchDestroyWindow(HWND window)
{
    const bool blocked = backgroundRequest();
    const BackgroundWindowChange change(blocked);
    noteForegroundLoss(window, true);
    if (tracePath[0])
    {
        char operation[48];
        std::snprintf(operation, sizeof(operation), "DestroyWindow:%s",
            window == GetForegroundWindow() ? "foreground" : "other");
        traceActivation(operation, blocked);
    }
...
static BOOL WINAPI launchSetWindowPos(HWND window, HWND after, int x, int y, int width, int height, UINT flags)
{
    ...
    if (blocked)
    {
        if (GetWindowLongPtrW(window, GWL_STYLE) & WS_CHILD) flags |= SWP_NOACTIVATE;
        else constrainBackgroundOrder(after, flags);
    }
    if (tracePath[0] && !(GetWindowLongPtrW(window, GWL_STYLE) & WS_CHILD))
    {
        char operation[64];
        std::snprintf(operation, sizeof(operation), "SetWindowPos:%s%s%s",
            (flags & SWP_NOZORDER) ? "keep" : after == HWND_BOTTOM ? "bottom" : after == HWND_TOP ? "top" : "other",
            (flags & SWP_SHOWWINDOW) ? "+show" : "", (flags & SWP_HIDEWINDOW) ? "+hide" : "");
        traceActivation(operation, blocked);
    }
    ...
    int x, int y, int width, int height, HWND parent, HMENU menu, HINSTANCE instance, LPVOID argument)
{
    ...
    const BackgroundWindowChange change(backgroundRequest());
...
static bool bindActivationImports(HMODULE module)
{
    ...
    const Binding bindings[] = {
        ...
        { reinterpret_cast<void*>(&SetWindowPlacement), reinterpret_cast<void*>(&launchSetWindowPlacement) },
        { reinterpret_cast<void*>(&DestroyWindow), reinterpret_cast<void*>(&launchDestroyWindow) },
    ...
    LPARAM lparam, UINT_PTR id, DWORD_PTR)
{
    ...
    {
        if (!backgroundWindowChangeDepth) InterlockedExchange(&userSelectedStudio, 1);
        if (GetPropW(window, deferredShowProperty))
    ...
    if (message == WM_WINDOWPOSCHANGING && backgroundWindowChangeDepth && backgroundRequest())
    {
        auto* position = reinterpret_cast<WINDOWPOS*>(lparam);
        // A later Qt show/raise can reorder a window without activating it.
        // Constrain only Studio's own background calls. Windows must remain
        // free to raise this window when the user selects it from another app.
        constrainBackgroundOrder(position->hwndInsertAfter, position->flags);
    ...
    if (code == HCBT_ACTIVATE || code == HCBT_CREATEWND)
    {
        ...
        if (protectedProcess)
        {
            ...
            if (code == HCBT_ACTIVATE)
            {
                ...
                const bool blocked = backgroundWindowChangeDepth
                    && !userActivation(activation) && shouldStayInBackground();
                if (tracePath[0])
                {
                    INPUT_MESSAGE_SOURCE source{};
            ...
            else
            {
                ...
                if (!(creation->lpcs->style & WS_CHILD))
                {
                    ...
                    if (backgroundRequest()) creation->hwndInsertAfter = HWND_BOTTOM;
