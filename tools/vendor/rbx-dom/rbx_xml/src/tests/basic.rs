...
#[test]
fn serialize_contentid_migrations_with_legacy_xml_property_names() {
    let _ = env_logger::try_init();

    let mesh_part = InstanceBuilder::new("MeshPart")
        .with_property("MeshId", ContentId::from("rbxassetid://93436871821239"))
        .with_child(
            InstanceBuilder::new("SurfaceAppearance")
                .with_property("ColorMap", ContentId::from("rbxassetid://90017515855512"))
                .with_property("MetalnessMap", ContentId::from("rbxassetid://123"))
                .with_property("NormalMap", ContentId::from("rbxassetid://456"))
                .with_property("RoughnessMap", ContentId::from("rbxassetid://789")),
        );
    let dom = WeakDom::new(mesh_part);
    ...
    assert!(!encoded.contains("MetalnessMapContent"), "{encoded}");
    assert!(!encoded.contains("NormalMapContent"), "{encoded}");
    assert!(!encoded.contains("RoughnessMapContent"), "{encoded}");
}
