...
pub(super) fn path_to_sourcemap_relative(project_root: &Path, path: &Path) -> String {
...
pub(super) fn load_existing_sourcemap_root(project_root: &Path) -> Result<Option<SourcemapNode>> {
...
pub(super) fn write_project_sourcemap_from_service_nodes(
    project_root: &Path,
...
fn insert_script_path_into_service_tree(
    ...
    let naming = project_config::cached_script_naming(file_path.parent().unwrap_or(project_root));
...
fn build_project_sourcemap_from_source(
    ...
    src_root: &Path,
    projected: Option<(
        &project_config::LoadedProject,
        &project_config::ProjectionStage,
    )>,
) -> Result<SourcemapNode> {
...
fn rewrite_projected_sourcemap_paths(
    ...
    if let Some(source) = projection.transformed_source_for_target(target) {
        ...
        for file_path in &mut node.file_paths {
            if let Some(source) =
                project_config::staged_path_to_project_source(loaded, Path::new(file_path))?
            {
...
pub(super) fn generate_sourcemap_command(
    ...
    let project_root = if let Some(project) = explicit_project.as_deref() {
        project_config::load_project(Some(project), None)?.root
    } else {
    ...
    loop {
        let loaded = if let Some(project) = explicit_project.as_deref() {
            Some(project_config::load_project(Some(project), None)?)
        } else {
...
fn sourcemap_watch_inputs(
    ...
    if let Some(loaded) = loaded {
        for path in project_config::project_source_roots(loaded)? {
            directories.insert(absolutize_for_daemon(&path));
        }
        for path in project_config::project_config_paths(loaded)? {
