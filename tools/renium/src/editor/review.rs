...
pub(crate) fn is_engine_managed_editor_property(
    class_name: &str,
    property_name: &str,
...
pub(crate) fn is_externally_managed_protected_write(row: &Value) -> bool {
...
pub(crate) fn property_schema_entry<'a>(
...
pub(crate) fn editor_review_payload(changes: &EditorChangeSet) -> (u64, Vec<Value>) {
...
pub(crate) fn request_editor_push_review(
    bridge: &BridgeServer,
...
pub(crate) fn request_protected_write_review(
    bridge: &BridgeServer,
    rows: &[Value],
) -> Result<bool> {
    if rows.is_empty() {
...
pub(crate) fn protected_write_matches_previous(row: &Value) -> bool {
...
pub(crate) fn protected_write_rows_with_previous_values(
    path: &Path,
    rows: &[Value],
) -> Result<Vec<Value>> {
...
#[cfg(target_os = "macos")]
pub(crate) fn studio_title_for_bridge(bridge: &BridgeServer, _pid: u32) -> Result<String> {
    Ok(bridge
        .cached_bridge_info_for_target(BridgeTarget::Edit)?
        .place_name)
}
