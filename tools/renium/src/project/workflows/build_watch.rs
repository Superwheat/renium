...
use crate::project::config::{self, LoadedProject};
use crate::system::files::{absolutize_for_daemon as absolute_path, path_extension_is};
...
fn roblox_ts_config_graph(root: &Path) -> Result<RobloxTsConfigGraph> {
    fn visit(
        ...
        let value = config::parse_jsonc_value(&text)
...
fn project_watch_inputs_into(
    ...
    for path in nested {
        ...
        if (name.ends_with(".project.json") || name.ends_with(".project.jsonc")) && path.is_file() {
            let nested = config::load_project(Some(&path), None)?;
...
pub(super) fn watch_build(
    ...
    loop {
        ...
        if project_graph_changed {
            match config::load_project(Some(&loaded.path), None) {
                Ok(current) => {
                    *loaded = current;
