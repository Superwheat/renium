...
fn apply_update(
    ...
    let managed_studio_platform = update_plugin
        .then(crate::studio_native_serializer::source_studio_platform_key)
        .transpose()?;
    #[cfg(not(target_os = "macos"))]
    let managed_studio_platform: Option<String> = None;
    #[cfg(target_os = "macos")]
    if let Some(target_platform) = managed_studio_platform.as_deref() {
        crate::studio_native_serializer::managed_studio_path()?;
        if manifest
            .payload
            .components
            .get(target_platform)
...
fn snapshot_managed_studio(stage: &Path) -> Result<ManagedStudioBackup> {
    let target = crate::studio_native_serializer::managed_studio_path()?;
...
fn recover_running_core_install() -> Result<()> {
    ...
    #[cfg(target_os = "macos")]
    crate::studio_native_serializer::recover_managed_studio_install()?;
    Ok(())
}
...
fn recover_pending_update_transaction(lifecycle_lock: &LifecycleLock) -> Result<Option<String>> {
    ...
    crate::workflows::stop_all_daemons_for_update()?;
    #[cfg(windows)]
