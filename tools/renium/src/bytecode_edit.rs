...
use super::file_io::{ensure_existing_ancestor_inside, exact_path_key, path_key};
...
pub(super) struct BytecodeCloneRefMap {
    pub(super) new_index_by_old: HashMap<usize, usize>,
    pub(super) old_index_by_settings_id: HashMap<String, usize>,
    pub(super) old_index_by_debug_id: HashMap<String, usize>,
    pub(super) old_index_by_path_key: HashMap<String, Option<usize>>,
    pub(super) old_index_by_path_parts_key: HashMap<String, usize>,
}

pub(super) fn collect_settings_subtree_preorder(
...
pub(super) fn bytecode_service_name(
...
pub(super) fn unique_editor_child_name(
    document: &SettingsBytecode,
    child_indices: &[usize],
    requested: &str,
) -> String {
...
pub(super) fn next_editor_settings_id_fast(
...
pub(super) struct CloneRefMapInput<'a> {
    ...
    pub(super) path_segments_before: &'a [Option<Vec<String>>],
    pub(super) path_ordinals_before: &'a [Option<Vec<usize>>],
}

pub(super) fn build_clone_ref_map(
    document: &SettingsBytecode,
...
pub(super) fn remap_internal_clone_refs_in_record(
...
pub(super) fn strict_ref_old_index(
    object: &Map<String, Value>,
    refs: &BytecodeCloneRefMap,
) -> Result<Option<usize>> {
...
pub(super) fn path_segments_from_value(value: &Value) -> Option<Vec<String>> {
...
pub(super) fn instance_path_key(path_segments: &[String]) -> String {
    path_segments.join("\0")
}

pub(super) fn instance_path_parts_key(path_segments: &[String], path_ordinals: &[usize]) -> String {
...
pub(super) fn insert_unique_rbx_path(
...
pub(super) fn path_ordinals_from_value(value: &Value) -> Option<Vec<usize>> {
    value
        .as_array()?
        .iter()
        .map(|item| {
...
pub(super) fn prune_empty_source_dirs(service_dir: &Path, start: &Path) -> Result<()> {
...
pub(super) fn has_direct_package_link_child(
    document: &SettingsBytecode,
    children_by_parent: &[Vec<usize>],
    target_index: usize,
) -> bool {
