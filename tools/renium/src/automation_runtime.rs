...
use crate::studio_automation::{
    ...
    goto_result, key_result, press_result, shot_result, start_stop_play_result,
...
fn automation_bootstrap_context(
    ...
    let project_path = project_root.join(project_config::PROJECT_FILE_NAME);
    let fingerprint =
...
fn automation_bind(
    ...
    let direct_project = explicit_project
        .clone()
        .unwrap_or_else(|| root.join(project_config::PROJECT_FILE_NAME));
    ...
    let loaded = project_config::load_project(explicit_project.as_deref(), Some(&root)).map_err(
        |error| automation::Failure::new("no_project", format!("{error:#}"), false, "project-init"),
    )?;
...
impl Drop for AutomationSelection {
    fn drop(&mut self) {
        ...
        crate::runtime_context::clear_automation();
    }
}
...
fn select_automation_context(context: &automation::BoundContext) -> AutomationSelection {
    ...
    crate::runtime_context::select_automation(
...
fn automation_requires_runtime(operation: u16, parameters: &Value) -> bool {
    matches!(operation, 10..=16 | 44 | 53..=66)
...
fn automation_response(
    ...
    let result = if matches!(request.op, 0 | 1 | 2 | 3 | 50 | 51 | 80 | 82) {
...
pub(super) fn automation_parse_response(
    text: &str,
    state: &automation::State,
    bridge: &BridgeServer,
    bridge_wait_seconds: f64,
...
pub(super) fn oversized_automation_request_response() -> automation::Response {
    automation::Response::failure(
        0,
        Instant::now(),
        automation::Failure::new(
...
pub(super) fn run_automation_stdio(
    bridge: &BridgeServer,
    state: &automation::State,
    bridge_wait_seconds: f64,
) -> Result<()> {
