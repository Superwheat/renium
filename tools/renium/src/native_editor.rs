...
pub(super) fn decode_bridge_buffer(
    value: &Value,
    expected_len: usize,
    label: &str,
) -> Result<Vec<u8>> {
...
pub(super) struct EditorBinaryExportFinishGuard<'a> {
    pub(super) bridge: &'a BridgeServer,
    pub(super) export_id: Option<String>,
}

impl EditorBinaryExportFinishGuard<'_> {
    pub(super) fn finish(&mut self, record_sync_completion: bool) -> Result<bool> {
...
pub(super) fn rbx_variant_referent(value: &RbxVariant) -> Option<RbxRef> {
    match value {
...
pub(super) struct NativeServiceDom {
...
pub(super) fn editor_binary_export_parts<'a>(
...
#[cfg(any(windows, target_os = "macos", test))]
pub(super) fn rbx_dom_service_root_property_values(
    dom: &RbxWeakDom,
...
#[cfg(any(windows, target_os = "macos"))]
pub(super) fn read_place_service_root_property_values(
    path: &Path,
    service_names: &HashSet<String>,
    database: &ReflectionDatabase<'_>,
...
#[cfg(any(windows, target_os = "macos"))]
pub(super) fn read_live_service_root_property_values(
    bridge: &BridgeServer,
    service: &str,
    database: &ReflectionDatabase<'_>,
    ...
    let result = (|| -> Result<Map<String, Value>> {
        let snapshot = studio_native_serializer::write_live_service(pid, &title, service, &path)?;
...
#[cfg(any(windows, target_os = "macos", test))]
pub(super) fn merge_live_service_root_property_values(
    service: &str,
...
#[cfg(any(windows, test))]
pub(super) fn encode_service_root_property_values(
    service: &str,
...
#[cfg(windows)]
pub(super) fn write_live_editor_place_snapshot(
    bridge: &BridgeServer,
...
pub(super) fn wait_for_editor_review_decision(
    bridge: &BridgeServer,
    response: Value,
    change_count: u64,
    label: &str,
...
pub(super) fn send_editor_change_batches(
    bridge: &BridgeServer,
