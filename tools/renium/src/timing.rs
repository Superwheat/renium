...
pub(super) fn current_millis() -> u128 {
    SystemTime::now()
...
pub(super) fn log_timing(label: &str, started: Instant) {
...
pub(super) fn log_timing_ms(label: &str, elapsed_ms: f64) {
...
pub(super) fn quiet_timings() -> bool {
    QUIET_TIMINGS.load(Ordering::Relaxed)
}

pub(super) fn set_quiet_timings(quiet: bool) {
    QUIET_TIMINGS.store(quiet, Ordering::Relaxed);
}

pub(super) fn verbose_timing_logs() -> bool {
