...
pub(super) fn validate_luau_syntax(code: &str) -> Result<()> {
...
pub(super) fn execute_luau_result(args: ExecuteLuauArgs, bridge: &BridgeServer) -> Result<Value> {
...
pub(super) fn studio_device_command(args: StudioDeviceArgs) -> Result<()> {
...
pub(super) fn studio_device_resolution(raw: &str) -> Result<(u32, u32)> {
...
pub(super) fn studio_device_result(
...
pub(super) fn start_stop_play_command(args: StartStopPlayArgs) -> Result<()> {
...
pub(super) fn studio_change_state_command(args: StudioChangeStateArgs) -> Result<()> {
...
pub(super) fn studio_change_state_result(
...
pub(super) fn start_stop_play_result(
...
fn read_client_mouse_location(bridge: &BridgeServer, player: Option<&str>) -> Result<(f64, f64)> {
    ...
    match (x, y) {
        ...
        _ => bail!("getMouseLocation returned no coordinates: {result}"),
    }
}
...
fn resolve_player_window(
    ...
    let windows = input_inject::list_studio_windows()?;
    let mut marks = Vec::with_capacity(windows.len());
    for (index, window) in windows.iter().enumerate() {
        let mark_x = 173 + (index as i32) * 37;
        let mark_y = 131 + (index as i32) * 17;
        input_inject::post_mouse_move(window, mark_x, mark_y)?;
        marks.push((mark_x, mark_y));
    }
    thread::sleep(Duration::from_millis(120));
    let (seen_x, seen_y) = read_client_mouse_location(bridge, player)?;
    let mut best: Option<(usize, f64)> = None;
    for (index, (mark_x, mark_y)) in marks.iter().enumerate() {
        let dx = *mark_x as f64 - seen_x;
        let dy = *mark_y as f64 - seen_y;
        let distance = (dx * dx + dy * dy).sqrt();
        if best.is_none_or(|(_, previous)| distance < previous) {
            best = Some((index, distance));
        }
    }
    let (index, distance) = best.context("No Studio windows to calibrate against")?;
    if distance > 24.0 {
        bail!(
            "Could not match the target client to a Studio window: the client reported mouse \
             position ({seen_x:.0}, {seen_y:.0}) but no calibration mark landed nearby. Candidate \
             windows: {}",
            windows
                .iter()
    ...
    let (mark_x, mark_y) = marks[index];
    let offset_x = mark_x - seen_x.round() as i32;
    let offset_y = mark_y - seen_y.round() as i32;
    let window = windows.into_iter().nth(index).expect("index in range");
    Ok((window, offset_x, offset_y))
}

pub(super) fn press_result(args: &PressArgs, bridge: &BridgeServer) -> Result<Value> {
...
fn read_client_probe_state(
    ...
    let position = match (
        ...
        result.get("probeY").and_then(Value::as_f64),
    ) {
...
fn calibrate_click_delta(
    ...
    if input_inject::post_probe_click(window, x, y).is_err() {
        return (0, 0);
    }
    for _ in 0..15 {
        thread::sleep(Duration::from_millis(60));
        match read_client_probe_state(bridge, player) {
            Ok((seq, Some((seen_x, seen_y)))) if seq != start_seq => {
    ...
    println!("[renium] warning: input calibration probe click was not observed by the client");
    (0, 0)
}

pub(super) fn click_result(args: &ClickArgs, bridge: &BridgeServer) -> Result<Value> {
    let player = args.player.as_deref();
    if let Some(player) = player {
        wait_for_player_bridge(bridge, player, args.bridge.wait_seconds)?;
    }
...
pub(super) fn key_result(args: &KeyArgs, bridge: &BridgeServer) -> Result<Value> {
    let player = args.player.as_deref();
    if let Some(player) = player {
        wait_for_player_bridge(bridge, player, args.bridge.wait_seconds)?;
    }
...
pub(super) fn ui_result(args: &UiArgs, bridge: &BridgeServer) -> Result<Value> {
    let player = args.player.as_deref();
    if let Some(player) = player {
        wait_for_player_bridge(bridge, player, args.bridge.wait_seconds)?;
    }
...
pub(super) fn type_result(args: &TypeArgs, bridge: &BridgeServer) -> Result<Value> {
    let player = args.player.as_deref();
    if let Some(player) = player {
        wait_for_player_bridge(bridge, player, args.bridge.wait_seconds)?;
    }
...
pub(super) fn goto_result(args: &GotoArgs, bridge: &BridgeServer) -> Result<Value> {
    let player = args.player.as_deref();
    if let Some(player) = player {
        wait_for_player_bridge(bridge, player, args.bridge.wait_seconds)?;
    }
...
pub(super) fn goto_command(args: GotoArgs) -> Result<()> {
...
pub(super) fn shot_result(args: &ShotArgs, bridge: &BridgeServer) -> Result<Value> {
    let player = args.player.as_deref();
    if let Some(player) = player {
        wait_for_player_bridge(bridge, player, args.bridge.wait_seconds)?;
    }
    ...
    let (width, height) = input_inject::capture_window_png(&window, &output)?;
    Ok(json!({
        "ok": true,
        "action": "shot",
...
pub(super) fn press_command(args: PressArgs) -> Result<()> {
    let mut daemon_args = Vec::new();
    if let Some(path) = args.path.as_ref() {
...
pub(super) fn click_command(args: ClickArgs) -> Result<()> {
...
pub(super) fn key_command(args: KeyArgs) -> Result<()> {
    let daemon_args = vec![
        args.key.clone(),
        "--hold-ms".to_string(),
...
pub(super) fn ui_command(args: UiArgs) -> Result<()> {
...
pub(super) fn type_command(args: TypeArgs) -> Result<()> {
...
pub(super) fn wait_until_command(args: WaitUntilArgs) -> Result<()> {
...
pub(super) fn shot_command(args: ShotArgs) -> Result<()> {
    let output = if args.output.is_absolute() {
        args.output.clone()
    } else {
        std::env::current_dir()
...
pub(super) fn list_clients_command(args: ListClientsArgs) -> Result<()> {
...
pub(super) fn editor_review_decision_result(
...
pub(super) fn editor_review_decision_command(args: EditorReviewDecisionArgs) -> Result<()> {
...
pub(super) fn test_command(args: TestArgs) -> Result<()> {
...
pub(super) struct TestLaunch {
    ...
    pub(super) edit_runtime_id: String,
}
...
pub(super) fn timed_test_result(args: TestArgs, bridge: &BridgeServer) -> Result<Value> {
    if !matches!(args.mode.as_str(), "play" | "run" | "server") {
        bail!(
            "Invalid test mode '{}'; use play, run, or server",
            args.mode
